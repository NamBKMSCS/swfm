...
1. Create an environment file:
   ...
   ```
2. (Optional) Edit `apps/ml-service/.env` to customize:
...
- Logs written to `apps/ml-service/logs/`
- MLflow UI: `http://localhost:5000`
- API Health: `http://localhost:8000/health`
...
```bash
cd apps/ml-service
bash start_ml_service.sh stop
bash start_ml_service.sh status
bash start_ml_service.sh restart
```
...
## Logs
- Local script run: `apps/ml-service/logs/mlflow.log`, `api_service.log`, `auto_training.log`
- Docker Compose:
  ```bash
